...
describe('buildFrameSequence', () => {
  ...
  it('omits the contributions shading legend regardless of show_legend', () => {
    ...
    for (const sequence of [withLegend, withoutLegend]) {
      expect(sequence.frames[0]?.svg).not.toContain('Below tip (3+)');
      expect(sequence.frames[0]?.svg).not.toContain('Unoccupied');
      expect(sequence.frames[0]?.svg).not.toContain('sc-legend');
    }
  });
...
/** Counts image descriptors while skipping colour tables and compressed data. */
function countFrames(buffer: Uint8Array): number {
  const skipSubBlocks = (start: number): number => {
    let offset = start;
    while (offset < buffer.length) {
      const size = buffer[offset] ?? 0;
      offset += 1;
      if (size === 0) break;
      offset += size;
    }
  ...
  const globalPacked = buffer[10] ?? 0;
  let offset =
    13 + (globalPacked & 0x80 ? 3 * 2 ** ((globalPacked & 0x07) + 1) : 0);
  let count = 0;
  while (offset < buffer.length) {
    const marker = buffer[offset];
    if (marker === 0x2c) {
      count += 1;
      const packed = buffer[offset + 9] ?? 0;
      offset += 10 + (packed & 0x80 ? 3 * 2 ** ((packed & 0x07) + 1) : 0) + 1;
      ...
      throw new Error(`Unexpected GIF block marker: ${String(marker)}`);
    }
  }
  return count;
}
