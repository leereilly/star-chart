...
describe('independent axes and legends', () => {
      ...
      for (const x of [false, true])
        for (const y of [false, true])
          for (const legend of [false, true]) {
            ...
            expect(svg.includes('class="sc-legend"')).toBe(
              style === 'contributions' ? false : legend,
            );
          }
    },
  );
  ...
  it('omits the contributions legend and reclaims its reserved space', () => {
    const shown = renderChart(model('contributions')).svg;
    const hidden = renderChart(
      model('contributions', { show_legend: 'false' }),
    ).svg;
    expect(shown).not.toContain('class="sc-legend"');
    for (const label of [
      'Recorded stars',
      'Column tip',
      '1 below tip',
    ...
    expect(shown.match(/<svg[^>]* height="([^"]+)"/)?.[1]).toBe(
      hidden.match(/<svg[^>]* height="([^"]+)"/)?.[1],
    );
  });
  it('adds stars only to the default visible contributions title', () => {
    const defaultSvg = renderChart(model('contributions')).svg;
    const customSvg = renderChart(
      model('contributions', { title: 'Custom title' }),
    ).svg;
    const titleText = (svg: string) =>
      group(svg, 'sc-header').getElementsByTagName('text')[0]?.textContent;
    expect(titleText(defaultSvg)).toBe('octocat/hello-world stars');
    expect(defaultSvg).toContain(
      '<title id="sc-contributions-title">octocat/hello-world</title>',
    );
    expect(titleText(customSvg)).toBe('Custom title');
    expect(customSvg).toContain(
      '<title id="sc-contributions-title">Custom title</title>',
    );
  });
  ...
  it.each(styles)(
    ...
    (style) => {
      for (const adds of [[], [0], [10], [10, 0, 0]]) {
        ...
        expect(checkSvg(svg).errors).toEqual([]);
        if (!['contributions', 'sparkline'].includes(style))
