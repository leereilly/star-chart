...
function legendLayout(model) {
  ...
  const legend = legendEntries(model);
  if (legend.length === 0) return { height: 0, entries: [] };
  ...
  let x2 = 0;
  let row = 0;
  const entries = legend.map((entry) => {
...
function headerLines(model) {
  ...
  let statsY = 26;
  if (cfg.showTitle) {
    const visibleTitle = cfg.title ?? (cfg.style === "contributions" ? `${model.metadata.fullName} stars` : model.metadata.fullName);
    const fullTitle = Array.from(visibleTitle);
